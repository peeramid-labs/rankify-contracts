...
| `RankifyInstanceMainFacet`        | `createAndOpenGame`    |      Yes       | This function creates a new game and opens registration. It is now correctly guarded.                                                                 |
...
| `RankifyInstanceMainFacet`        | `startGame`            |       No       | This function starts a game. It does not make any external calls and is therefore not vulnerable to re-entrancy.                                      |
...
**Context:** The development team has acknowledged certain design trade-offs and has planned future improvements.

**Points of Interest:**

- **Memory Growth**: As noted by the team, storing all game data in the primary `RankifyInstance` contract could lead to unbounded memory growth, which is a potential DoS vector. The suggested architectural refactor to split game-specific state into separate contracts is the correct long-term solution.
...
- **Diamond Proxy Removal**: The team has noted that they plan to remove the Diamond Proxy in future patches to increase readability.
...
- **Unified Custom Errors**: The team has acknowledged the need to unify the custom error types used throughout the codebase.
...
- **`LibQuadraticVoting.sol`:**
  - **Infinite Loop:** The `do-while` loop in the `precomputeValues` function could potentially lead to an infinite loop if the `voteCredits` value is not a sum of squares. The `require` statement that is intended to prevent this is commented out in the provided code. This is a critical vulnerability that should be fixed.
...
- **`LibCoinVending.sol`:**
  ...
  - **`MockERC20`:** This library uses `MockERC20`, which is not a production-ready contract.

## 3. Areas for Auditor Scrutiny

This section details specific points of interest that the Peeramid team believes warrant close examination during the audit. It includes potential vulnerabilities identified during internal review and areas where the team would like an expert opinion.
...
-   **Integrity of the `MAODistribution` Instantiation Flow**:
    ...
    -   Auditors should scrutinize this flow for any potential manipulation vectors, such as re-entrancy or transaction-ordering attacks, that could lead to a partially configured or insecure MAO instance. While designed to be atomic, the team welcomes a thorough review of this critical function.
...
-   **`LibCoinVending` Hardening**:
    -   This library uses `MockERC20`. This is for testing purposes and will be replaced with a standard `IERC20` interface in production.
    -   The library uses `.transfer()` for sending Ether. The team is aware of the potential for this to fail and would like the auditors' recommendation on whether to migrate to the `call({value: ...})("")` pattern.
