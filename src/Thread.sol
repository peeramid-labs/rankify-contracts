...
contract Thread is ReentrancyGuardTransientUpgradeable, EIP712Upgradeable, IThread, IERC721Receiver, IERC1155Receiver {
    ...
    function initialize(
        ...
        LibCVPP.CVPP storage cvpp = LibCVPP.CVPPStorage();
    ...
    /**
     * @dev Handles the actions after the next turn.  `newProposals` is the array of new proposals.
     *
     * Modifies:
     *
     * - Sets the ongoing proposals to `newProposals`.
     * - Increments the number of ongoing proposals by the number of `newProposals`.
     */
    function _afterNextTurn(LibCVPP.CVPP storage cvpp, string[] memory newProposals) private {
        for (uint256 i = 0; i < newProposals.length; ++i) {
            cvpp.state.ongoingProposals[i] = newProposals[i];
        }
    }
    ...
    /**
     * @dev Handles the next turn of a thread.  `newProposals` is the array of new proposals.
     *
     * Emits an {OverTime_ event if the thread is in the last turn and overtime.
     * emits a _LastTurn_ event if the thread is in the last turn.
     * emits a _Over_ event if the thread is over.
     *
     * Modifies:
     *
     * - Calls the `_afterNextTurn` function with `newProposals`.
     */
    function _nextTurn(LibCVPP.CVPP storage cvpp,string[] memory newProposals) private {
        ...
        if (_hasEnded) {
            (address[] memory participants, uint256[] memory finalScores) = cvpp.close();
            emit Over(participants, finalScores);
        }
        _afterNextTurn(cvpp, newProposals);
    }
    ...
    function endTurn(
        ...
        if (turn != 1) {
            // 1. Handle previous turn's voting and scoring
            {
                ...
                for (uint256 i = 0; i < participants.length; i++) {
                    require(permutation[i] < participants.length, "Invalid proposer index");
                ...
                for (uint256 proposer = 0; proposer < participants.length; ++proposer) {
                    // permutation is the index where players proposal was shuffled in to
                    // uint256 proposalCol = permutation[proposer];
                    // We slice the votes array to get the votes for the current participant
                ...
                for (uint256 i = 0; i < participants.length; ++i) {
                    proposals[i] = cvpp.state.ongoingProposals[permutation[i]];
                }
                for (uint256 i = 0; i < participants.length; ++i) {
        ...
        }
        {
            ...
            require(
                ...
                "Too many participants, current ZKP circuit only supports 15 participants"
            );
            ...
            // Fill public inputs with commitments
            {
                ...
                for (uint256 i = participants.length; i < 15; ++i) {
        ...
        for (uint256 i = 0; i < participants.length; ++i) {
            cvpp.state.ongoingProposals[i] = "";
            cvpp.state.voted[participants[i]] = false;
            cvpp.state.ballotHashes[participants[i]] = bytes32(0);
            cvpp.state.proposalCommitment[participants[i]] = 0;
        }
        ...
        _nextTurn(cvpp, newProposals.proposals);
    }
    ...
    function isActive(address participant) public view returns (bool) {
        LibCVPP.CVPP storage cvpp = LibCVPP.CVPPStorage();
        return cvpp.isActive(participant);
    }
