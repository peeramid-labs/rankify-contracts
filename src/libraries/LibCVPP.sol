...
library LibCVPP {
    ...
    bytes32 private constant _PROPOSAL_PROOF_TYPEHASH =
        keccak256("signProposalByGM(uint256 turn,bytes32 proposalNHash,string encryptedProposal)");
    bytes32 private constant _VOTE_PROOF_TYPEHASH =
        keccak256("signVote(uint256 vote1,uint256 vote2,uint256 vote3,uint256 turn,bytes32 salt)");
    bytes32 private constant _VOTE_SUBMIT_PROOF_TYPEHASH =
        keccak256("publicSignVote(uint256 turn,bytes32 vote1,bytes32 vote2,bytes32 vote3)");
    ...
    bytes32 constant CVPP_STORAGE_POSITION = keccak256("cvpp.storage.position");

    function CVPPStorage() internal pure returns (CVPP storage es) {
    ...
    function init(CVPP storage cvpp, Settings memory newSettings) public {
        ...
        cvpp.state.rank = cvpp.state.rank;
        cvpp.state.rankClaimTimeLock = cvpp.state.rankClaimTimeLock;
        uint256 balance = cvpp.settings.rankToken.balanceOf(address(this), cvpp.state.rank + 1);
        if (balance == 0) {
            cvpp.settings.rankToken.mint(address(this), cvpp.state.rank + 1, 1, "");
            require(
                cvpp.settings.rankToken.balanceOf(address(this), cvpp.state.rank + 1) == 1,
                "LibCVPP::init->rankToken balance should be exactly 1 Rank token of rank above"
            );
        }
     ...
     */
    function canBeJoined(CVPP storage cvpp) public view returns (bool) {
    ...
    modifier onlyWhenTurnCanEnd(CVPP storage cvpp) {
        require(canEndTurn(cvpp) == true, "onlyWhenTurnCanEnd: Not everyone made a move yet and there still is time");
        _;
    }
    ...
    function attemptMove(CVPP storage cvpp, address participant) public onlyInTurnTime(cvpp) {
        ...
        // Set Participant as active when they make a move
        state.isActive[participant] = true;
        state.numActiveParticipants++;
    }
    ...
    function nextTurn(CVPP storage cvpp) public returns (bool, bool, bool) {
        ...
        // Update Participant activity status for next turn
        uint256 playerCount = state.participants.length();
        state.numActiveParticipants = 0;

        for (uint256 i = 0; i < playerCount; i++) {
            address participant = state.participants.at(i);
            // If Participant didn't make a move this turn, mark them as inactive
            if (!state.madeMove[participant]) {
                // console.log('LibCVPP::nextTurn - ','Participant inactive!');
                state.isActive[participant] = false;
            } else {
                // console.log('LibCVPP::nextTurn - ','Participant active!');
                state.numActiveParticipants++;
            }
            state.madeMove[participant] = false;
        }
    ...
    function close(
        ...
        require(
            block.timestamp - cvpp.state.startedAt >= cvpp.state.rankClaimTimeLock,
            "Game duration less than minimum required time"
        );

        (, uint256[] memory finalScores) = getScores(cvpp);
        address[] memory players = getParticipants(cvpp);
        for (uint256 i = 0; i < players.length; ++i) {
            removeParticipant(cvpp, players[i]);
        }
        emitRewards(cvpp);
        return (cvpp.state.leaderboard, finalScores);
    }
     ...
     */
    function emitReward(LibCVPP.CVPP storage cvpp) private {
        cvpp.state.winner = cvpp.state.leaderboard[0];
        if (cvpp.state.rank > 1) {
            cvpp.settings.rankToken.burn(cvpp.state.winner, cvpp.state.rank, 1);
        }
        cvpp.settings.rankToken.safeTransferFrom(address(this), cvpp.state.winner, cvpp.state.rank + 1, 1, "");
    }
     ...
     * Modifies:
     *
     * - Calls `emitReward` for the main rank and each additional rank in the game.
     */
    function emitRewards(LibCVPP.CVPP storage cvpp) public {
        emitReward(cvpp);
    }
    ...
    function getState(LibCVPP.CVPP storage cvpp) public view returns (LibCVPP.ThreadStateReturn memory) {
        return
            LibCVPP.ThreadStateReturn({
                ...
                rankClaimTimeLock: cvpp.state.rankClaimTimeLock,
    ...
    function calculateScores(
        ...
        for (uint256 i = 0; i < players.length; ++i) {
            playerVoted[i] = cvpp.state.isActive[players[i]];
        }
        uint256[] memory roundScores = cvpp.state.voting.tallyVotes(votesRevealed, playerVoted);
        for (uint256 playerIdx = 0; playerIdx < players.length; playerIdx++) {
            ...
            if (cvpp.state.proposalCommitment[players[playerIdx]] != 0) {
                ...
                setScore(cvpp, players[playerIdx], scores[playerIdx]);
            } else {
    ...
    function joinThread(CVPP storage cvpp, address player, bytes memory gameMasterSignature, bytes32 digest) public {
        ...
        if (cvpp.state.rank > 1) {
            cvpp.settings.rankToken.lock(player, cvpp.state.rank, 1);
        }
